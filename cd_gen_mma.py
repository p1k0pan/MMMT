...
def eval_model():
    ...
    for i, row in tqdm(df.iterrows(),total=df.shape[0]):
        text = row['question'] + " " + "Select the most suitable option for the question from the following choices. " + "A: " + row['A'] + ' | B: ' + row['B'] + ' | C: ' + row['C'] + ' | D: ' + row['D'] + "\nAnswer in format, like A | explanation."

        img = row["file_name"]

        outputs, log_sentence_prob = generate(text, image_folder + img)
...
if __name__ == "__main__":
    ...
    # ds = load_dataset("azizyooni/MMA", "main")["train"]
    df = pd.read_csv("/ltstorage/home/2pan/dataset/MMA_Anony/running.csv")
    image_folder = "/ltstorage/home/2pan/dataset/MMA_Anony/AMAMI/"
